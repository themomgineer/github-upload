...
async function onCardClick(e) {
   ...
   // Find the clicked element (e.target)
	...
	if (!isNaN(clickedID)) {   //"is Not a Number" (button id is just the card id number)
      var message = await setActiveCard(clickedID);
	...
	//toggle the card's select property
	   ...
	   var message = await setActiveCard(buttonID);
      //console.log("Active card is " + message);
...
async function onShuffleClick(e) {
   ...
   if(shuffleEl.checked) {
      ...
      //var ifSuccess = await db.decks.update(activeDeckID, {shuffle: false});
      var ifSuccess = await db.decks.where("id").equals(activeDeckID).modify({shuffle: true});

   }
   //Checkbox was unchecked, so set shuffle to false in db
   else {
      console.log("Checkbox was unchecked.")
      //var ifSuccess = await db.decks.update(Number(activeDeckID), {shuffle: false});
      var ifSuccess = await db.decks.where("id").equals(activeDeckID).modify({shuffle: false});
   }
   
   console.log("Updated shuffle? " + ifSuccess);
   checkDexie();
}
